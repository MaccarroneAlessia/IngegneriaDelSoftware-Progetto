...
public class Find implements StateFile {
    ...
    private String setpathmode(String txt) {
    ...
    private boolean highlight() {
        ...
        try {
            ...
            try (BufferedReader br = new BufferedReader(new InputStreamReader(in))) {
                ...
                while ((strLine = br.readLine()) != null) {
                    ...
                    if (strLine.contains(wordsFind.get(n - 1))) { // cerca l'ultima parola inserita nell'array delle
                                                                  // parole trovate
                        // se la riga è uguale a quella ricercata
                        String str = strLine.replace(wordsFind.get(n - 1), "***" + wordsFind + "***");
                        fileContent.append(str + System.getProperty("line.separator"));
        ...
        try { // chiusura dell'output e dell'input
            fstream.close();
            out.flush();
            out.close();
            in.close();
    ...
    private boolean modify(String txt) throws IOException {
        pathmod = setpathmode(txt); // Creare un nuovo file txt da modificare
